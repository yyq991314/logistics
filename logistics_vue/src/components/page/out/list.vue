<template>
    <div>
        ...
        <el-dialog :title="form._sn" :visible.sync="logVisible" width="50%">
            <el-form ref="log" :rules="rules" :model="form" label-width="120px">
                <div class="crumbs">
                    <el-breadcrumb separator="/">
                        <el-breadcrumb-item>
                            <i class="el-icon-lx-notice" style="color:#409EFF;"></i> 提示
...
<script>
    import {fetchList, revOrder, inOrder, outOrder, arrOrder} from '../../../api/out';
    import Qs from 'qs';
    ...
    export default {
        ...
        data() {
            return {
                ...
                activities: [],
                rules: {
                    logtypes: [{required: true, message: '请选择物流状态', trigger: 'blur'}],
                    logs: [{required: true, message: '请输入状态内容', trigger: 'blur'}],
                },
            };
        },
        created() {
        ...
        methods: {
            ...
            outOrder(index, row) {
                ...
                this.id = row.id;
                this.form = row;
                this.$confirm('确定要出库？', '温馨提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    if (this.form.orders != 3) {
                        this.$message.error("该订单物品已经出库");
                        return false;
                    }
                    this.form.username = localStorage.getItem('ms_username');
                    outOrder(this.form).then(res => {
                        if (res.status == 1) {
                            this.$message.success(res.msg);
                            this.form.orders = 4;
                            // this.tableData.splice(index, 1);
            ...
            submitLog() {
                this.$refs.log.validate(valid => {
                    if (valid) {
                        let v1 = this;
                        if (v1.form.logtypes == "" || v1.form.logtypes == undefined) {
                            v1.$message.error("请选择快件物流状态");
                            return false;
                        }
                        if (v1.form.logs == "" || v1.form.logs == undefined) {
                            v1.$message.error("请填写快件物流状态信息");
                            return false;
                        }
                        this.$axios({
                            method: "post",
