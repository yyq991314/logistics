    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>
                    <i class="el-icon-lx-cascades"></i> 已接单快递列表
                </el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
...
<script>
    import {fetchListed, takeOrder} from '../../../api/take';
    import Qs from 'qs';
    ...
    export default {
        ...
        methods: {
            ...
            takeOrder(index, row) {
                ...
                this.id = row.id;
                this.form = row;
                this.$confirm('确定要找人代取快递么？', '温馨提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    if (this.form.outtypes != 0) {
                        this.$message.error("该快递已经在派单代取中");
                        return false;
                    }
                    this.form.username = localStorage.getItem('ms_username');
                    takeOrder(this.form).then(res => {
                        if (res.status == 1) {
                            this.$message.success(res.msg);
                            this.form.outtypes = 1;
