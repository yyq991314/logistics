    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>
                    <i class="el-icon-lx-cascades"></i> 快递待取列表
                </el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
            ...
            >
                ...
                <el-table-column label="操作" width="160" align="center">
                    <template slot-scope="scope">
                        ...
                        <el-button
                                v-if="scope.row.orders==1"
                                type="text"
                                icon="el-icon-location-information"
                                class="red"
                                @click="rveOrder(scope.$index, scope.row)"
                        >确认送达
                        </el-button>
                        <el-button
                                v-if="scope.row.orders==2"
                                type="text"
                                icon="el-icon-location-information"
                                @click="inOrder(scope.$index, scope.row)"
                        >立即入库
                        </el-button>
                        <el-button
                                v-if="scope.row.orders == 3"
                                type="text"
                                icon="el-icon-location-information"
                                @click="outOrder(scope.$index, scope.row)"
                        >确认出库
                        </el-button>
                        <el-button
                                v-if="scope.row.orders == 4"
                                type="text"
                                icon="el-icon-location-information"
                        ...
                        <el-button
                                v-if="scope.row.orders == 5"
                                type="text"
                                icon="el-icon-location-information"
                                @click="arrOrder(scope.$index, scope.row)"
                        >网点揽收
                        </el-button>
                        <el-button
                                v-if="scope.row.orders == 6"
                                type="text"
                                icon="el-icon-location-information"
                                @click="takeOrder(scope.$index, scope.row)"
                        >找人代取
                        </el-button>
                        <el-button
                                v-if="scope.row.orders == 7"
                                type="text"
                                icon="el-icon-location-information"
                                @click="takeOrder(scope.$index, scope.row)"
                        >找人代取
            ...
            <el-form ref="form" :model="form" label-width="120px">
                <div class="crumbs">
                    <el-breadcrumb separator="/">
                        ...
                        <el-breadcrumb-item>更新物流状态</el-breadcrumb-item>
                    </el-breadcrumb>
                </div>
                <el-form-item label="物流状态">
                    <el-radio-group v-model="form.logtypes">
                        <el-radio label="1">运输中</el-radio>
                        <el-radio label="2">派件中</el-radio>
                    </el-radio-group>
                </el-form-item>
                <el-form-item label="状态内容">
                    <el-input v-model="form.logs"></el-input>
                </el-form-item>
                <el-form-item>
                    <el-button type="primary">更新物流</el-button>
                </el-form-item>
            </el-form>
            <div class="crumbs">
                <el-breadcrumb separator="/">
        ...
        <!-- 代取弹框 -->
        <el-dialog :title="form._sn" :visible.sync="takeVisible" width="50%">
            <el-form ref="take" :rules="rules" :model="form" label-width="120px">
                <div class="crumbs">
                    <el-breadcrumb separator="/">
                        <el-breadcrumb-item>
                            <i class="el-icon-lx-notice" style="color:#409EFF;"></i> 提示
                        </el-breadcrumb-item>
                        <el-breadcrumb-item>代取信息</el-breadcrumb-item>
                    </el-breadcrumb>
                </div>
                <el-form-item label="代取价格" prop="take_price">
                    <el-input v-model="form.take_price"></el-input>
                </el-form-item>
                <el-form-item>
                    <el-button type="primary" @click="saveTake">确定派单</el-button>
                </el-form-item>
            </el-form>
...
<script>
    import {fetchList, takeOrder} from '../../../api/take';
    import Qs from 'qs';
    import {checkNumber} from "../../../api/validator";
    ...
    export default {
        ...
        data() {
            return {
                ...
                rules: {
                    take_price: [
                        {required: true, message: '请输入代取价格', trigger: 'blur'},
                        {validator: checkNumber, message: '请输入正确的代取价格', trigger: 'blur'}
                    ],
                },
            };
        },
        ...
        methods: {
            ...
            saveTake() {
                this.$refs.take.validate(valid => {
                    if (valid) {
                        let v1 = this;
                        v1.$confirm('确定要找人代取快递么？', '温馨提示', {
                            confirmButtonText: '确定',
                            cancelButtonText: '取消',
                            type: 'warning'
                        }).then(() => {
                            if (this.form.outtypes != 0) {
                                v1.$message.error("该快递已经在派单代取中");
                                return false;
                            }
                            v1.form.username = localStorage.getItem('ms_username');
                            takeOrder(v1.form).then(res => {
                                if (res.status == 1) {
                                    v1.$message.success(res.msg);
                                    v1.form.outtypes = 1;
                                    v1.$set(v1.tableData, v1.idx, v1.form);
                                    // v1.tableData.splice(v1.idx, 1);
                                    v1.takeVisible = false;
                                } else {
                                    v1.$message.error(res.msg);
                                }
                            });
                        }).catch(() => {
                        });
            ...
            submitApp() {
                let v1 = this;
                v1.form.apptime = v1.form.apptime1;
                if (v1.form.apptime == "" || v1.form.apptime == undefined) {
                    v1.$message.error("请选择预约取件时间");
                    return false;
                }
                if (v1.form.appmark == "" || v1.form.appmark == undefined) {
                    v1.$message.error("请填写预约备注");
                    return false;
                }
                this.$axios({
                    method: "post",
                    url: "/pick/apptime.html",
                    headers: {
            ...
            endOrder(index, row) {
                ...
                this.id = row.id;
                this.form = row;
                this.form._sn = "物流运单号 " + this.form.sn;
