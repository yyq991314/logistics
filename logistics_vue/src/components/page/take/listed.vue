    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>
                    <i class="el-icon-lx-cascades"></i> 待送达列表
                </el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
...
<script>
    import {orderListed, endOrder} from '../../../api/take';
    import Qs from 'qs';
    ...
    export default {
        ...
        methods: {
            // 获取 easy-mock 的模拟数据
            getData() {
                orderListed(this.query).then(res => {
            ...
            endOrder(index, row) {
                ...
                this.id = row.id;
                this.form = row;
                this.$confirm('确定签收么？', '温馨提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    if (this.form.orders != 8) {
                        this.$message.error("该订单物品已经签收");
                        return false;
                    }
                    this.form.username = localStorage.getItem('ms_username');
                    endOrder(this.form).then(res => {
                        if (res.status == 1) {
