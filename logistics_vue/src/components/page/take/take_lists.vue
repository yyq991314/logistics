    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>
                    <i class="el-icon-lx-cascades"></i> 代取快递派单列表
                </el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
                ...
                <el-table-column label="操作" width="160" align="center">
                    <template slot-scope="scope">
                        ...
                        <el-button
                                ...
                                class="red"
                                @click="takeOut(scope.$index, scope.row)"
                        >立即抢单
                        </el-button>
                    </template>
                </el-table-column>
...
<script>
    import {takeList, takeOut} from '../../../api/take';
    import Qs from 'qs';
    ...
    export default {
        ...
        methods: {
            ...
            takeOut(index, row) {
                ...
                this.id = row.id;
                this.form = row;
                this.$confirm('确定要代取该快递么？', '温馨提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    if (this.form.orders != 6 && this.form.orders != 7) {
                        this.$message.error("该快递已经被接单");
                        return false;
                    }
                    this.form.username = localStorage.getItem('ms_username');
                    takeOut(this.form).then(res => {
                        if (res.status == 1) {
