    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>
                    <i class="el-icon-lx-cascades"></i> 快递代发抢单
                </el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
                ...
                <el-table-column label="操作" width="160" align="center">
                    <template slot-scope="scope">
                        ...
                        <el-button
                                ...
                                class="red"
                                @click="ordersRop(scope.$index, scope.row)"
                        >立即抢单
                        </el-button>
                    </template>
                </el-table-column>
        ...
        <el-dialog title="订单详情" :visible.sync="editVisible" width="50%">
            <el-form ref="form" :model="form" label-width="120px">
                ...
                <el-form-item>
                    <el-button type="primary" @click="onSubmit">立即抢单</el-button>
                </el-form-item>
            </el-form>
...
<script>
    import {fetchOrder, ropOrder} from '../../../api/logistics';
    ...
    export default {
        ...
        methods: {
            // 获取 easy-mock 的模拟数据
            getData() {
                fetchOrder(this.query).then(res => {
            ...
            ordersRop(index, row) {
                ...
                this.id = row.id;
                this.form = row;
                this.$confirm('确定要代发该快递？', '温馨提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    if (this.form.orders != 0) {
                        this.$message.error("该订单已经接单");
                        return false;
                    }
                    this.form.username = localStorage.getItem('ms_username');
                    ropOrder(this.form).then(res => {
                        if (res.status == 1) {
